...
namespace SBS_Ecommerce
{
    public class MvcApplication : HttpApplication
    {
        ...
        protected void Application_AcquireRequestState(object sender, EventArgs e)
        {
            ...
            {
                if (context.HttpContext.Session["Master"] == null)
                {
                    if (context.HttpContext.Session["Error_Page"] == null)
                    {
                        var host = HttpContext.Current.Request.Url.AbsoluteUri;
                        if (host.Contains("/master") || host.Contains("/Master"))
                        {
                            context.HttpContext.Session["Master"] = true;
                            ctx.Response.Redirect(urlHelper.Content("~/Master/UploadTheme"));
                        }
                        else
                        {
